<template>
  <div class="mr-5 flex items-center justify-center">
    <label for="dropzone-file"
      class="p-2 flex flex-col items-center justify-center w-full h-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-200"
      @drop.prevent="onDrop" @dragover.prevent.stop @dragleave.prevent.stop accept="image/*">
      <div v-if="flag" class="flex flex-col items-center justify-center gap-4">
        ...
        <div>
          <p class="text-center mb-2 text-sm text-gray-500 dark:text-gray-400">
            <span class="font-semibold">Нажмите, чтобы загрузить</span> или перетащите файл сюда
          </p>
          <p class="text-center text-xs text-gray-500 dark:text-gray-400">SVG, PNG, JPG (3x4)</p>
        </div>
      </div>

      <img class="object-fill h-full w-full" v-else :src="photo" alt="your-photo">

      <input id="dropzone-file" type="file" class="hidden" @change="handleInput" maxlength="1" multiple="false"
        accept="image/*" />
    </label>
  </div>
</template>
...
const handleInput = (e: Event) => {
  ...
  if (target.files && target.files[0]) readFile(target.files[0]);
}
</script>
